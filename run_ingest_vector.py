"""Run Stage 4: Ingest obligation records into ChromaDB.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 4: Vector DB Ingest")
    parser.add_argument("--version", default="v1.0.0", help="Dataset version to ingest")
    parser.add_argument("--input", default=None, help="Path to raw JSONL (default: data/output/raw_{version}.jsonl)")
    ...
    count = run_vector_ingest_stage(records, settings)
    print(f"Vector ingest complete: {count} records")
